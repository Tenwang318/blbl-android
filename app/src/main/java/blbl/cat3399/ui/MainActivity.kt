...
class MainActivity : BaseActivity(), SidebarFocusHost {
    ...
    private var isSidebarExpanded: Boolean = false
    ...
    private val sidebarReturnFocus = FocusReturn()
    ...
    private fun syncSidebarExpansionWithPrefs() {
        // Sidebar is now an overlay toggled by Start; don't auto-expand from prefs.
            ...
            when (event.keyCode) {
                KeyEvent.KEYCODE_DPAD_LEFT -> {
                    if (focused != null && isInMainContainer(focused)) {
                        ...
                        // Only enter sidebar via DPAD_LEFT when sidebar is already visible.
                        // When hidden, user must use Start to open sidebar.
                        if (isSidebarExpanded && canEnterSidebarFrom(focused)) {
                            releaseDpadDownFocusGuard()
    ...
    private fun switchToPrevTab() {
        val rootFragment = currentRootFragment() ?: return
        val fragmentView = rootFragment.view ?: return
        val tabLayout = fragmentView.findViewById<com.google.android.material.tabs.TabLayout?>(R.id.tab_layout) ?: return
        if (tabLayout.tabCount <= 1) return
        val cur = tabLayout.selectedTabPosition.takeIf { it >= 0 } ?: 0
        val prev = if (cur <= 0) tabLayout.tabCount - 1 else cur - 1
        switchTabAndFocus(tabLayout, prev)
    }
    ...
    private fun switchToNextTab() {
        ...
        if (tabLayout.tabCount <= 1) return
        val cur = tabLayout.selectedTabPosition.takeIf { it >= 0 } ?: 0
        val next = if (cur >= tabLayout.tabCount - 1) 0 else cur + 1
        switchTabAndFocus(tabLayout, next)
    }

    private fun switchTabAndFocus(tabLayout: com.google.android.material.tabs.TabLayout, position: Int) {
        val rootFragment = currentRootFragment() ?: return
    ...
    private fun focusMainFromSidebar(): Boolean {
        ...
        // When sidebar is an overlay, moving focus to main should close it.
        val shouldCollapseSidebar = isSidebarExpanded

        fun armSidebarCollapseAfterFocusTransfer() {
            if (shouldCollapseSidebar) {
                setSidebarExpanded(expanded = false)
            } else if (BiliClient.prefs.mainAutoHideSidebarOnEnterContent) {
    ...
    private fun setSidebarExpanded(expanded: Boolean) {
        ...
        val sidebarWidth = binding.sidebar.width.toFloat().takeIf { it > 0 }
            ?: resources.getDimension(R.dimen.sidebar_width)

        if (expanded) {
            // Save current main focus before opening sidebar overlay
            val focused = currentFocus
            if (focused != null && isInMainContainer(focused)) {
            ...
            binding.sidebar.animate()
                ...
                .withEndAction {
                    binding.sidebar.visibility = View.GONE
                    // Restore focus to main content after sidebar closes
                    sidebarReturnFocus.restoreAndClear(fallback = null, postOnFail = true)
                }
                .start()
        }
    }
